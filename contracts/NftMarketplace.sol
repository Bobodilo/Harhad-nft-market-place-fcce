...
error NftMarketplace__NotApprvedForMarketPlace();
...
contract NftMarketplace is ReentrancyGuard {
    ...
    // NftAddres => Nft toeknID => Seller and Selling Price
     ...
     * @param nftAddress: Address of thr NFT
    ...
    function listItem(
        ...
        // with Chainlink pricefeed, convert the price of the token between each other
        ...
        isOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        if (nft.getApproved(tokenId) != address(this)) {
            revert NftMarketplace__NotApprvedForMarketPlace();
        }
        s_listings[nftAddress][tokenId] = Listing(price, msg.sender);
        emit ItemListed(msg.sender, nftAddress, price, tokenId);

        // How do we store the NFTs? Array or Mapping?
        // I think mapping cos we can keep track of which address owns what
    }
    ...
    function buyItem(
        ...
        //let the msg.sender get the item (Pull over Push)
        IERC721(nftAddress).safeTransferFrom(listedItem.seller, msg.sender, tokenId);
        //Make sure the nft was transfered
